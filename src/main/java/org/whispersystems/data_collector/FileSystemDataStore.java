...
public class FileSystemDataStore implements DataStore {
  ...
  @Override
  public void store(InputStream data, String callId, String clientId, String dataSource, int attemptId)
      throws IOException{
    ...
    File outFile = new File(storageDir, "calldata-" + dataSource + "-" + attemptId + ".json.gz");
    if (outFile.exists()) {
      throw new IOException("File exists: " + outFile);
    }

    Path dataFile = outFile.toPath();
